...
class TodoTabBarController: UITabBarController {
    ...
    @SceneDependencyReference var viewManager: TodoScene.ViewManager?
        ...
        Task {
            ...
            viewModel
                ...
                .sink(receiveValue: { [weak self] _ in
                    ...
                    Task {
                        await self.viewManager?.showTodoDetail()
                    }
                })
                .store(in: &cancellables)
    ...
    @objc func showNewTodoAlert() {
        ...
        vc.addAction(UIAlertAction(title: "OK", style: .default) {
            _ in
            Task {
                await self.viewModel?.dispatch(type: .createTodo, payload: self.newTodo as Any)
            }
        })
    ...
    @objc func logout() {
        Task {
            await viewModel?.dispatch(type: .logout, payload: 0)
        }
    }
